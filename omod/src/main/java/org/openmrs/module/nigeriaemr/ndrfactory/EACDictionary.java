...
public class EACDictionary {
    ...
    private EACType createEACType(Patient patient, Date visitDate, List<Obs> obsPerVisit) throws DatatypeConfigurationException{
        ...
        obs = Utils.extractObs(166097, obsListForOneVisit); //Eac Session type
        if (obs != null && obs.getValueCoded() != null) {
            valueCoded = obs.getValueCoded().getConceptId();
            ndrCode = getMappedValue(valueCoded);
            eacType.setEACSessionType(ndrCode);
        }
    ...
    public String getMappedValue(int value_coded) {
        if (eacSessionMap.containsKey(value_coded)) {
            return eacSessionMap.get(value_coded);
        }
        return null;
    }
